...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Processing ${action} for request ${requestId} by user ${user.id}`
    );
    ...
    if (!isCoordinator) {
      return new Response(
        JSON.stringify({
          error: "Only coordinators can approve/reject skip requests",
        }),
        {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (action === "approve") {
      ...
      if (studentQueue.profiles.status !== "active") {
        return new Response(
          JSON.stringify({
            error: "Student is inactive - cannot perform swap",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      if (excludedProfileIds.length > 0) {
        eligibleStudentQuery = eligibleStudentQuery.not(
          "profile_id",
          "in",
      ...
      const { data: eligibleStudent, error: eligibleError } =
        await eligibleStudentQuery.limit(1).maybeSingle();
      ...
      if (eligibleError || !eligibleStudent) {
        ...
        return new Response(
          JSON.stringify({
            error:
              "No eligible student available for swap. All students after position 10 have already been involved in skip approvals today or are inactive.",
          }),
          {
            status: 404,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      const { error: updateReqError } = await supabase
        ...
        .update({
          ...
          review_notes:
            reviewNotes ||
            `Swapped position ${studentQueue.queue_position} with ${eligibleStudent.profiles.full_name} at position ${eligibleStudent.queue_position}`,
        })
        .eq("id", requestId);
      ...
      if (swapError) {
        ...
        return new Response(
          JSON.stringify({
            error: "Failed to swap positions - request reverted to pending",
          }),
          {
            status: 500,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      const { data: tomorrowAssignment, error: assignmentError } =
        await supabase
      ...
      if (tomorrowAssignment && !assignmentError) {
        ...
        const updatedProfileIds = tomorrowAssignment.profile_ids.map(
          (id: string) => {
